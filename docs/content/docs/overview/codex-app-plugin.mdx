...
description: Install and use the Jackie Canvas plugin in Codex App
---

# Codex App Plugin

The plugin connects Codex to a local Canvas Agent, allowing Codex to open Jackie Canvas, read and create nodes, connect nodes, and start generation workflows. Its source is in `plugins/infinite-canvas/`.
...
```text
Install the Jackie Canvas Codex plugin from https://github.com/basketikun/infinite-canvas.git.
...
In a new Codex conversation, ask it to open Jackie Canvas, create a canvas, and connect Codex. The plugin starts Canvas Agent, reads its connection URL and token, opens `https://canvas.best/`, and creates and connects a canvas. It starts a local frontend only when explicitly requested.
