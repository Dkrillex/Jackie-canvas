...
- Canvases, assets and generation history stay in browser storage; there is no cloud sync for them.
- The optional `server/` service (Alipay top-up, credit wallet and the jobs marketplace) is the one part that requires a backend and a database. Without it the rest of the app runs unchanged.
