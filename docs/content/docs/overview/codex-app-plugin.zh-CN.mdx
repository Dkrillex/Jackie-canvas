---
title: Codex App 插件
description: 在 Codex App 中安装并使用 Jackie Canvas 插件
---

# Codex App 插件

Jackie Canvas 提供了一个 Codex App 插件，用来把 Codex 和本地 Canvas Agent 连接起来。安装后，你可以直接在 Codex 中说“打开 Jackie Canvas”，让 Codex 打开画布、读取节点、创建节点并触发生成流程。
...
```text
请从 https://github.com/basketikun/infinite-canvas.git 安装 Jackie Canvas Codex 插件。
...
确保 `~/.agents/plugins/marketplace.json` 中有 Jackie Canvas 条目。这里的 `path` 要指向仓库里的插件子目录：
...
如果你正在 Jackie Canvas 仓库中开发插件，可以直接使用仓库自带的 marketplace：
...
```text
打开 Jackie Canvas 新建画布 并连接好Codex
```
