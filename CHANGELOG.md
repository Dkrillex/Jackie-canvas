...
+ [优化][Jackie] 从文本节点拉线新建图片节点时自动带上文本作为提示词，生成时不再重复拼接同一段文本。
+ [修复][Jackie] 画布里在工具栏、侧栏、节点面板上双指捏合不再放大整页，工具栏不会再忽大忽小或被推出屏幕。
+ [修复][Jackie] Seedance 素材库图片插入画布后改用长期有效的 OSS 原图地址，不再隔天裂图，也能作为 gpt-image 编辑等需上传文件的参考图，不再报「读取图片失败」。
+ [优化][Jackie] 画布生成中回到首页或其它页时不再卸载画布，请求可在后台跑完；整页刷新或换另一张画布仍会中断（视频任务刷新后仍可续查）。
+ [修复][Jackie] 反推提示词不再把 Seedance 的 `asset://` 当 base64 发给对话接口。
+ [修复][Jackie] 画布局部遮罩编辑按原图像素发尺寸，不再默认生成正方形。
+ [调整][Jackie] 钱包充值档位改为一行四列。
+ [调整][Jackie] 兑换记录说明改为「额度已发放 N 积分」，不再展示 add_quota 内部额度。
+ [新增][Jackie] 充值增加 30 元 / 4 积分档。
+ [调整][Jackie] 商务联系邮箱和 QQ 改为 1658176374。
...
+ [调整][Jackie] 首页标题下那行说明小字浅色下改成深靛 #3f3c64；「画布」的下划线与「内容」的遮罩块统一换成 #4F5BDB（遮罩 40% 透明度，下划线不再压透明度），深色模式配色不变。
