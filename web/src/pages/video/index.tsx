...
export default function VideoPage() {
    ...
    const buildRequestSnapshot = () => {
        ...
        if (!text) {
            message.error(t("wb.needVideoPrompt"));
            return null;
        }
    ...
    const saveResultToAssets = (video: GeneratedVideo) => {
        addAsset({
            kind: "video",
            title: t("wb.videoAssetTitle"),
            coverUrl: "",
            tags: [],
            source: t("wb.source.video"),
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig, agentTaskId?: string) => {
        ...
        try {
            for (let attempt = 0; attempt < 120; attempt += 1) {
                ...
                if (attempt === 119) throw new Error(t("wb.videoTimeout"));
                    ...
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-6 space-y-5">
                            <div>
                                ...
                                <Input.TextArea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={7} placeholder={t("wb.videoPromptPh")} />
                            </div>
...
function filterAudioReferencesByDuration(existing: ReferenceAudio[], next: ReferenceAudio[], warn: (content: string) => void) {
    ...
    if (skipped) warn(useLocaleStore.getState().t("wb.ignoreAudioDuration"));
    return accepted;
}
