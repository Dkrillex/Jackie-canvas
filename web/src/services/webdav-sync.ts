...
export async function uploadWebdavFile(config: WebdavSyncConfig, path: string, file: Blob, contentType = "application/octet-stream") {
    if (!file.size) throw new Error("Upload cancelled: file is empty");
    await ensureWebdavDirectory(config);
...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit) {
    ...
    try {
        ...
        if (error instanceof Error && error.name === "AbortError") throw new Error("WebDAV request timed out — check network or remote service status");
        if (error instanceof TypeError) throw new Error("Cannot connect to WebDAV — check URL, HTTPS certificate, CORS, or network");
        throw error;
    } finally {
...
function assertWebdavConfig(config: WebdavSyncConfig) {
    if (!config.url.trim()) throw new Error("Please enter a WebDAV URL first");
}
...
async function throwWebdavError(response: Response, fallback: string): Promise<never> {
    ...
    if (response.status === 404) throw new Error("WebDAV path not found — check URL and remote directory");
