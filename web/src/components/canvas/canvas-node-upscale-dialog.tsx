...
const algorithms: Array<{ value: ImageUpscaleAlgorithm; titleKey: MessageKey; descriptionKey: MessageKey }> = [
    { value: "high", titleKey: "canvas.dialog.algoHigh", descriptionKey: "canvas.dialog.algoHighDesc" },
    { value: "bilinear", titleKey: "canvas.dialog.algoBilinear", descriptionKey: "canvas.dialog.algoBilinearDesc" },
    { value: "nearest", titleKey: "canvas.dialog.algoNearest", descriptionKey: "canvas.dialog.algoNearestDesc" },
];
...
export function CanvasNodeUpscaleDialog({
    dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageUpscaleParams) => void }) {
    const { t } = useI18n();
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={820} centered destroyOnHidden>
            <div className="space-y-5">
                <div>
                    <h2 className="text-xl font-semibold">{t("canvas.dialog.upscaleTitle")}</h2>
                </div>
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_360px]">
                    <div className="rounded-xl border p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                            <span className="opacity-60">{t("canvas.dialog.source")}</span>
                            <span className="font-semibold">{image ? `${image.width} x ${image.height} px` : t("canvas.dialog.reading")}</span>
                        </div>
                    </div>
                    <div className="space-y-6 py-2">
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">{t("canvas.dialog.targetPx")}</div>
                            <Segmented
                                block
                            ...
                            />
                            {image && !canUpscale ? <div className="text-xs font-medium text-[#ef4444]">{reachedMax ? t("canvas.dialog.atMax") : t("canvas.dialog.atTarget")}</div> : null}
                        </div>
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">{t("canvas.dialog.algo")}</div>
                            <Segmented
                                block
                        ...
                        <div className="rounded-xl border px-4 py-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="opacity-60">{t("canvas.dialog.outputSize")}</span>
                                <span className="font-semibold">{outputSize ? `${outputSize.width} x ${outputSize.height} px` : t("canvas.dialog.unknown")}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="flex justify-end">
                    <Button type="primary" size="large" icon={<ImagePlus className="size-4" />} disabled={!canUpscale} onClick={() => onConfirm(params)}>
                        {t("canvas.dialog.generateUpscale")}
                    </Button>
                </div>
            </div>
        </Modal>
