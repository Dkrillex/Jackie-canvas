...
export function CanvasNodeSplitDialog({
    dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageSplitParams) => void }) {
    const { t } = useI18n();
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={780} centered destroyOnHidden>
            <div className="space-y-5">
                <div>
                    <h2 className="text-xl font-semibold">{t("canvas.dialog.splitTitle")}</h2>
                    <p className="mt-1 text-sm opacity-60">{t("canvas.dialog.splitDesc", { n: total })}</p>
                </div>
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
                    <div className="rounded-xl border p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                            <span className="opacity-60">{t("canvas.dialog.original")}</span>
                            <span className="font-semibold">{image ? `${image.width} x ${image.height} px` : t("canvas.dialog.reading")}</span>
                        </div>
                    </div>
                    <div className="space-y-5 py-2">
                        <NumberField label={t("canvas.dialog.rows")} value={rows} onChange={(value) => update("rows", value)} />
                        <NumberField label={t("canvas.dialog.cols")} value={columns} onChange={(value) => update("columns", value)} />
                        <div className="grid grid-cols-2 gap-2">
                            <Button icon={<Rows3 className="size-4" />} onClick={() => addLine("horizontal")}>{t("canvas.dialog.hLine")}</Button>
                            <Button icon={<PanelTop className="size-4 rotate-90" />} onClick={() => addLine("vertical")}>{t("canvas.dialog.vLine")}</Button>
                            <Button icon={<Trash2 className="size-4" />} disabled={!active} onClick={deleteLine}>{t("canvas.dialog.delLine")}</Button>
                            <Button icon={<ListRestart className="size-4" />} onClick={resetLines}>{t("canvas.dialog.resetLines")}</Button>
                        </div>
                        <div className="rounded-xl border px-4 py-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="opacity-60">{t("canvas.dialog.sliceCount")}</span>
                                <span className="font-semibold">{t("canvas.dialog.countUnit", { n: total })}</span>
                            </div>
                            <div className="mt-2 flex items-center justify-between">
                                <span className="opacity-60">{t("canvas.dialog.avgAbout")}</span>
                                <span className="font-semibold">{pieceSize ? `${pieceSize.width} x ${pieceSize.height}` : t("canvas.dialog.unknown")}</span>
                            </div>
                        </div>
                        <Button type="primary" size="large" className="w-full" icon={<Grid2x2 className="size-4" />} onClick={() => onConfirm(confirmParams)}>
                            {t("canvas.dialog.generateChildren")}
                        </Button>
                    </div>
                </div>
            </div>
