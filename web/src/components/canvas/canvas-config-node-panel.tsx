...
export function CanvasConfigNodePanel({
    node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    const { t } = useI18n();
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between gap-3">
                <div className="shrink-0 text-sm font-semibold">{t("canvas.tool.config")}</div>
            ...
            <div className="mb-2 flex flex-wrap gap-1.5">
                <InputChip label={t("canvas.node.prompt")} value={t("canvas.config.countItems", { n: inputSummary.textCount })} style={chipStyle} />
                <InputChip label={t("canvas.config.refImage")} value={t("canvas.node.count", { n: inputSummary.imageCount })} style={chipStyle} />
                <InputChip label={t("canvas.config.refVideo")} value={t("canvas.config.countItems", { n: inputSummary.videoCount })} style={chipStyle} />
                <InputChip label={t("canvas.config.refAudio")} value={t("canvas.config.countItems", { n: inputSummary.audioCount })} style={chipStyle} />
                ...
                <span className="inline-flex items-center gap-1.5">
                    {isRunning ? (
                        <>
                            ...
                            <span>{t("canvas.prompt.stop")}</span>
                        </>
                    ) : (
                        <>
                            <Play className="size-4" />
                            <span>{t("canvas.config.start")}</span>
                        </>
                    )}
                </span>
            </Button>
