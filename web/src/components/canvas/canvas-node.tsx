...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const finishTitleEditing = useCallback(() => {
        const title = titleDraft.trim() || data.title || useLocaleStore.getState().t("canvas.node.unnamed");
        setTitleDraft(title);
            ...
            {(isSelected || hovered || isEditingTitle) && (
                ...
                {isEditingTitle ? (
                    ...
                    <button
                        ...
                        title={t("canvas.node.editTitleHint")}
                        onDoubleClick={(event) => {
                            event.stopPropagation();
                            setIsEditingTitle(true);
                        }}
                    >
                        {data.title || t("canvas.node.unnamed")}
                    </button>
                )}
                </div>
            )}
        ...
        <div className="pointer-events-none flex h-full w-full flex-col p-4">
            <div className="flex items-center gap-2 text-sm font-semibold" style={{ color: theme.node.text }}>
                ...
                <span className="ml-auto rounded-full px-2 py-1 text-[11px] font-medium" style={{ background: theme.node.fill, color: theme.node.muted }}>
                    {t("canvas.node.childCount", { n: groupChildCount ?? 0 })}
                </span>
            </div>
...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    ...
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            <div className="text-xs leading-5 text-red-300">{node.metadata?.errorDetails || t("canvas.node.genFailed")}</div>
            <button
                type="button"
...
function MissingPluginContent({ theme, type }: Pick<NodeContentRendererProps, "theme"> & { type: string }) {
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-4 text-center" style={{ color: theme.node.placeholder }}>
            ...
            <span className="text-sm">{t("canvas.node.unknownNode")}</span>
    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden pt-8">
            <button
                ...
                title={t("canvas.node.textToImage")}
                aria-label={t("canvas.node.textToImage")}
            >
                <ImageIcon className="size-3.5" />
                {t("canvas.node.genImageShort")}
            </button>
            {isEditingContent ? (
                ...
                <div
                    ...
                    onWheel={(event) => event.stopPropagation()}
                >
                    {node.metadata?.content || <span style={{ color: theme.node.placeholder }}>{t("canvas.node.editTextHint")}</span>}
                </div>
            )}
        </div>
    );
    ...
    return (
        <div className="flex h-full w-full flex-col justify-center gap-3 px-4" style={{ background: theme.node.fill, color: theme.node.text }}>
            <div className="flex min-w-0 items-center gap-2 text-sm opacity-70">
                ...
                <span className="truncate">{t("common.audio")}</span>
            </div>
