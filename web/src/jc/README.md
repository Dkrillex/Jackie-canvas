...
仓库根部的 `server/`（充值与接单服务）、`vite.config.ts` 的 `/pay-api` / `/new-api` / `/gw` 代理、
`middleware.js`、`vercel.json`、`hinnflow-docker.conf` 也属于二开范围，但它们要么是新增文件、
要么是几行基础设施配置，不在这张表里。
...
jc/
  ...
  config.ts       前端环境常量（如 /pay-api 基址、AGENT_UI_ENABLED、LOCAL_PROXY_UI_ENABLED）
...
企业管理仍挂 MaaS `/prod-api`，和 New API 登录不是同一套凭证。入口已隐藏：顶栏没有「企业」，
登录不探测 MaaS，`/enterprise` 深链回首页。页面和接口客户端仍在 `pages/enterprise/`、
`services/enterprise.ts`，有身份映射之前不要接回去。
