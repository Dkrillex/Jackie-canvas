...
- `illucent-dev`：功能与 `tennda-dev-one` 一致（含 `tennda-*` api id / 网关映射）；对外品牌为 Illucent AI；展示名用 Illucent；主题为紫白（主色 `#7C5CFC`，参考 SiliconFlow 色系）；Logo 用 `web/public/logo.svg`；不要回写成 TENNDA AI 蓝系或旧 logo。
