...
- Seedance 视频模型在 `/gw` 下按模型名识别（含 `seedance` / `doubao-seedance`），不要只依赖 `apiFormat === "ark"`；时长需落在 Seedance 合法范围（2.0 一般为 4–15 或 -1），界面不要用 OpenAI 的 4–30 秒滑杆。现网 ID 带 `doubao-` 前缀和日期：`doubao-seedance-2-0-260128`、`doubao-seedance-2-0-fast-260128`、`doubao-seedance-2-0-mini-260615`，不要发不带前缀的短名或 NSFW 变体。生视频走 `POST /gw/api/v3/contents/generations/tasks` 再 `GET .../tasks/{id}`，不要再打 `/v1/video/generations`。图生视频：首尾帧模式发 `first_frame`/`last_frame`，全能参考才发 `reference_image`，两种 mode 不要混。fast / mini 最高 720p，不要给这两档发 1080p。素材库现网走 REST `GET/POST /gw/v1/asset-groups` 与 `/gw/v1/assets`：文档里的 `ListAssetGroups` / `ListAssets` Action 上游 BytePlus 返回 UnsupportedAction，不要改去只用 Action 列列表。参考图能发 URL 的（Seedance / Gemini / 对话识图）直接发已编码的 https，不要转 base64；有 `storageKey` 用本地 blob；只有 multipart 上传文件时才在浏览器里下载，OSS bucket `super-jackie` 已开 CORS `*`。素材库接口的 `url` 是火山 TOS 预签名（`X-Tos-Expires=41400` 约 11.5 小时、无 CORS），不要存进节点也不要在浏览器 fetch；预览和节点 `content` 用列表里的 `gravitex_url`（上传时的 OSS 原图，单个 `GET /assets/{id}` 不返回这个字段）。
