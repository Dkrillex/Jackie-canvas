---
name: open-canvas
description: 打开 Jackie Canvas 在线或本地画布，并自动连接本地 Canvas Agent。用户要求打开、启动、进入或使用 Jackie Canvas 画布时使用。
---

# Open Jackie Canvas

当用户要求打开、启动、进入或使用 Jackie Canvas 时，不要把 URL 交给用户手动复制，不要通过浏览器点击“新建画布”。默认打开在线版；只有用户明确要求使用本地项目时，才启动本地前端。
...
1. 在 Jackie Canvas 项目中启动前端，并使用 Vite 输出的 `Local` 地址：
...
2. 启动本地 Canvas Agent，必须带上第 1 步得到的真实画布地址：

```bash
CANVAS_URL=<Vite Local 地址> npx -y @jackie-canvas/canvas-agent
```
...
插件在新的 Codex 任务中加载时会自动启动 `npx -y @jackie-canvas/canvas-agent mcp`。这个 MCP 进程负责提供画布工具，不提供网页连接服务；
