...
import { alipayConfigured, mysqlConfigured, novaAdminConfigured, novaApiMysqlConfigured, settings } from "./config.js";
...
import { QuotaGrantError } from "./nova-quota.js";
import { NovaApiDatabaseNotConfigured } from "./nova-users.js";
...
app.use(
    ...
    cors({
        ...
        allowHeaders: ["Content-Type", "Authorization", "New-Api-User"],
...
app.onError((error, c) => {
    ...
    if (error instanceof NovaApiDatabaseNotConfigured) return c.json({ success: false, message: error.message }, 503);
    ...
    if (error instanceof QuotaGrantError) return c.json({ message: error.message }, error.status as 502);
    console.error(`[pay] 未处理的错误 ${c.req.method} ${c.req.path}：${error.stack || error.message}`);
    return c.json({ message: error.message || "服务异常" }, 500);
});

app.get("/health", (c) =>
    c.json({ ok: true, alipay: alipayConfigured(), mysql: mysqlConfigured(), novaApi: novaApiMysqlConfigured(), novaAdmin: novaAdminConfigured() }),
);
